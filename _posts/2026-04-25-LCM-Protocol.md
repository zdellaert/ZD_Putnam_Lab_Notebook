---
layout: post
title: Laser Capture Microdissection of Paxgene-Fixed Cryo-Embedded Tissue Protocol
date: '2026-04-25 7:00:00'
categories: Protocols, Processing
tags: [Pocillopora, Porites, LCM]
---

# LCM Protocol for Paxgene-fixed tissues

## First step:

- Process tissues as written in the [PFCE protocol](https://github.com/zdellaert/ZD_Putnam_Lab_Notebook/blob/master/_posts/2025-09-10-PFCE-Protocol.md)

## This protocol requires two special pieces of equiment not used in my normal tissue sectioning/downstream protocols:

### PEN Slides

- We are using 4 µm Frame PEN slides (steel frames, Leica Cat #11600289)
  - We ordered these directly from Leica but they are available other places also ([example with good picture](https://lifesciences.danaher.com/us/en/products/sku/11600289-leica.html))
- See notes about different LCM slide options from Leica [here](https://www.leica-microsystems.com/science-lab/life-science/consumables-for-laser-microdissection/) and [here](https://github.com/zdellaert/ZD_Putnam_Lab_Notebook/blob/master/protocols/lmdslidememo.pdf)

### LMD7 microscope

- [See info here](https://www.leica-microsystems.com/products/light-microscopes/p/leica-lmd7/)

## Sectioning for LCM

### Before sectioning: sample + slide prep (prepare PEN slide with UV and RNAse cleaner)
...
3. Follow this by two 15 second rinses in DEPC (RNAse-free) water
...
[Section](https://zdellaert.github.io/ZD_Putnam_Lab_Notebook/Cryosectioning-Protocol/) onto PEN membrane slide for LCM as well as extra slides for confocal & H&E morpholgy imaging and backup for RNA and DNA extraction
...
10. Make glass slides for confocal/H&E morphology (do these first):
    1.  First label and make a few glass slides for confocal/H&E morphology visualization
...
11. When confident: **Section onto PEN Slide for LCM**
    1.  Label slide -- be careful with ethanol as it will make the sharpie run
    2.  Keep slide cold in the cryostat, briefly warm sections with finger on back of slide to adhere. Refreeze on cryostat plate.
    3.  Once all sections are adhered, dry the slide at room temp in a sterile petri dish for 1 min
    4.  Then, inside the dry ice cooler, pipette on ice-cold 100% ethanol and let sit 2 minutes
    5.  Remove excess ethanol, gently blotting bottom of slide with kimpwipe but minimizing any lint contamination
    6.  Transfer slide to falcon tube with dessicant, let sit out at room temp in this sealed tube for 15 minutes
    7.  Bury sealed tube in dry ice and keep in dry ice until transfer to -80 ºC freezer.
12. Remove block from chuck with razor blade or forceps. Cover exposed tissue with OCT, freeze, and keep in dry ice until transfer to -80 ºC.
13. Remove clingwrap, clean cryostat with 70% ethanol, dispose of blades in sharps bin.

## Staining

**In lab: Staining Procedure** (modified/combined from [here](https://github.com/zdellaert/ZD_Putnam_Lab_Notebook/blob/master/protocols/leicalmdprotocolguide-May-2015.pdf) and [here](https://github.com/zdellaert/ZD_Putnam_Lab_Notebook/blob/master/protocols/Zeiss-RNA-extraction-frozen-sections.pdf) to reduce exposure to aqueous solutions of lower than 70% ethanol to reduce RNAses) ([ALSO](https://www.olivelab.org/uploads/6/3/6/2/6362060/laser_capture_microdissection_on_frozen_sections_for_extraction_of_high-quality_nucleic_acids.pdf))

*Solutions to prepare and chill on ice before staining* (**ALL SOLUTIONS ICE COLD**):
   - Cresyl violet staining solution (in 100% ethanol)
   - 70% ethanol
   - 100% ethanol
   - RNAse-free water
...
## Testing Alt. Staining protocols
...
2. Post-stain: 75% ethanol, ice-cold
   1. 30 mL ethanol + 10 mL RNAse-free H2O
3. Post-stain: 96% ethanol, ice-cold
...
1. Clean working space and all scope parts (slide holder, sample collectors, computer mouse, focus/stage knobs) with 70% ethanol and RNAse cleaner using damp Kimwipes.
   1. *Do not spray anything onto the equipment.*
2. Load slide onto slide holder (thick part with label goes towards the spring holder)
   1. **Slide should be loaded right-side up, with tissue on the bottom**
3. Label 4 PCR tubes and put in the collector
4. Fill caps of each tube with 40 µL digestion buffer. Record order in lab notebook.
   1. **Note order carefully and know which tube is A, B, C, & D on the LCM**
5. Load collector onto scope
6. In the LasX software, open two windows: Laser control, microscope control
7. Take overview image of slide at 5X magnification
8. Change magnification to 20X for dissection
9. Set laser settings 
   1.  power = 25 μJ, speed = 10, pulse frequency = 500 Hz, brightfield illumination
10. Test dissection in an area of the membrane without tissue to make sure laser is working
11. In notebook, assign tissue type 1 (e.g. oral epidermis) to one tube and tissue type 2 (e.g. aboral epidermis) to another tube per coral fragment
12. For each dissection
    1.  Select the correct tube in the bottom left
    2.  Draw the ROI containing the tissue you want to dissect
        1.  make sure to leave some space between the line and the target tissue border to account for the laser width
    3.  Double check the color of the ROI matches the tube color for that tissue type
    4.  **Before dissecting, enable scale bar & take image with ROI visible (SAVE + L), and rename image file with tube # and dissection #**
    5.  Hit GO! And hopefully the area is dissected as planned and the membrane + tissue fall out of view
    6.  Take another image post-dissection with ROI visible (SAVE + L), and rename image file with tube # and dissection # ("POST")
    7.  Switch to collector view to inspect cap for dissected tissue
    8.  If the dissection was successful, record ROI area (µm²) in lab notebook for each dissection
        1.  We will later measure dissected tissue areas from these images in FIJI.
    9.  If the dissection was not successful, you can use the laser to re-cut any membrane that is still hanging on, or if the tissue got destroyed somehow or very burnt by the laser, just move on and note that this dissection was not successful
    10. *You can draw multiple ROIs for different tubes in one view, just make sure to switch tubes between ROIs. And when going from one view/dissection to the next, always confirm the tube selected for collection before dissecting.*
13. Work quickly; once a section is done, carefully remove tubes and immediately close caps
    1.  Remove the tubes with the dissections VERY CAREFULLY, closing the cap as you remove it.
    2.  Post-collection: Spin down tube, vortex 5-10 s, spin down 30 s minimum, then place in tube rack on dry ice.
14. After slide is done, place back in falcon tube with desiccant and transport on dry ice to -80°C storage.
...
[Leica LCM RNA testing](https://www.leica-microsystems.com/science-lab/life-science/workflows-protocols-how-to-use-a-leica-laser-microdissection-system-and-qiagen-kits-for-successful-rna-analysis/)
